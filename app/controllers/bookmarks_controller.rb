class BookmarksController < ApplicationController
  ...
  def create
    ...
    the_bookmark.crawl_id = params.fetch("query_crawl_id")
    the_bookmark.rating = params.fetch("query_rating")

    if the_bookmark.valid?
      the_bookmark.save
      redirect_to("/", { :notice => "Bookmark created successfully." })
    else
      redirect_to("/", { :alert => the_bookmark.errors.full_messages.to_sentence })
    end
  end
